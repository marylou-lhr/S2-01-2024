...
void Diaporama::chargerImages(Images& pImages) {
    ...
    if (db->openDataBase()) {
        qDebug() << "OK BD";
        QSqlQuery query;
        if (query.exec("SELECT D.titrePhoto, F.nomfamille, D.uriPhoto FROM Diapos D JOIN Familles F ON F.idFamille = D.idFam")) {
            while (query.next()) {
                Image* imageACharger = new Image();
                imageACharger->creationImage(
                    query.value(1).toString().toStdString(),
                    query.value(0).toString().toStdString(),
                    ":/diapo" + query.value(2).toString().toStdString()
                );
                pImages.push_back(imageACharger);
            }
            qDebug() << "Images chargées depuis la base de données";
        } else {
            qDebug() << "Échec de l'exécution de la requête SQL:" << query.lastError();
        }
        db->closeDataBase();
        qDebug() << "ferme BD";
    } else {

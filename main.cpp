...
int main()
{
    ...
    while (true)
    {
        ...
        unsigned int position = diaporamas[diaporamaCourant]._placeImages[imageCourante].getPos();
        lecteur.afficherImageCourante (diaporamas[diaporamaCourant], imageCourante, images[position]);

        /* Menu des actions possibles (saisie choix utilisateur) :
         * A-vancer, R-eculer, C-hanger de diaporama, Q-uitter */

        lecteur.verifChoix(choixAction);
        if (choixAction == 'Q')
        {
            break;
        }
        ...
        system("cls");  // effacer l'écran
        lecteur.choixAction(choixAction, diaporamas, diaporamaCourant, imageCourante, images);
    }
